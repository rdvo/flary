...
A research function has a clear input and output. Start it as a durable run
when the caller can disconnect before the report is ready.
...
Resolve `approved-sources` in trusted host code. Its credentials do not enter
the prompt or tool schema.
