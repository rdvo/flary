...
Use a function for one question. Use an agent when a support ticket needs many
messages, tools, approvals, or child investigations.
...
Your application owns login, customer identity, billing, and the screen. Flary
owns model execution, tools, validation, and durable runs.
...
Store the product ticket ID with `thread.ref.threadId`. Reopen that thread for
each reply. See [Build your UI or bot](/docs/clients/) for the transport
pattern.
