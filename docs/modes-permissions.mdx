...
~~~ts
import { flary, z, resolveAgentMode } from "flary";

const app = flary<Env>({
  name: "support",
  bindings: z.object({
    AGENTS: z.any(),
    THREAD_METADATA: z.any(),
  }),
});
...
export const support = app.fn({
  ...
  mode: "support-agent",
  prompt: async ({ message }, { bindings }) => {
    const thread = await bindings.THREAD_METADATA.read();
    const revision = await bindings.AGENTS.get(thread.agentId, thread.revisionId);
    const mode = resolveAgentMode(thread.mode, revision.customModes);
...
important rule is that the host resolves the mode during admission and
recovery, not from model input. The `mode` option records the operational mode
ID with the function revision. A mode change is also a semantic history
checkpoint, so operators can see who changed it and why.
