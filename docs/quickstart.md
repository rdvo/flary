...
Flary opens `http://127.0.0.1:43817`. The setup creates a local project and deploys it to your
Cloudflare account. It does not send credentials to a Flary service.
...
If no public client ID is set, select **Use Wrangler OAuth**. Wrangler owns that OAuth client and
callback. Flary asks Wrangler to use the operating system keychain when the installed version
supports it. This is the safest supported fallback for an unconfigured source checkout.
...
Run the same command again with the same project directory. Flary reads `.flary/project.json`,
`.flary/quickstart.json`, and generated source files. It never stores the provider key in the setup
record. Enter the key again if `.dev.vars` is missing.
