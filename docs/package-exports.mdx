...
description:
  Find the right Flary module for prompts, durable runs, tools, providers, storage, and Cloudflare
  adapters.
section: Reference
order: 1
---

Import from a focused entry point. This keeps application bundles and type declarations small and
makes the host boundary easy to audit.
...
Run the clean npm installation test in CI so a host does not silently lose checked-in dependency
patches.
