...
class MemoryStatement {
  ...
  async first<T>(): Promise<T | null> {
    ...
    if (this.query.includes("kind = 'mcp' AND namespace = ?")) {
      ...
      const row = [...this.database.connections.values()].find(
        (item) =>
  ...
  async all<T>(): Promise<{ results: T[] }> {
    if (this.query.includes("FROM flary_connection")) {
      ...
      const results = [...this.database.connections.values()].filter(
        (row) =>
  ...
  async run(): Promise<{ success: boolean; meta: { changes: number } }> {
    ...
    if (this.query.startsWith("INSERT INTO flary_mcp_oauth_session")) {
      const [id, tenantId, userId, connectionId, privateState, expiresAt, createdAt, updatedAt] =
        this.#values;
    ...
    if (this.query.startsWith("UPDATE flary_mcp_oauth_session SET status = ?")) {
      ...
      Object.assign(requiredRow(this.database.flows, id), {
        status,
        private_state:
          status === "pending" ? requiredRow(this.database.flows, id).private_state : "",
        updated_at: updatedAt,
      });
      return changed();
    }
...
test("MCP URL setup discovers OAuth, completes PKCE, and exposes namespaced tools", async () => {
    ...
    if (url === "https://mcp.example.com/mcp") {
      if (!headers.has("authorization")) {
        return new Response(null, {
          ...
          headers: {
            "www-authenticate":
              'Bearer resource_metadata="https://mcp.example.com/.well-known/oauth-protected-resource", scope="tools.read"',
          },
        });
      }
  ...
  const pending = await service.start(scope, {
    name: "Product Search",
