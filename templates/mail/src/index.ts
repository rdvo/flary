...
app.get("/health", (context) =>
  context.json({ ok: true, service: "flary-mail" })
);

app.on(["GET", "POST"], "/api/auth/*", (context) =>
  createAuth(context.env, new URL(context.req.url).origin).handler(
    context.req.raw
  )
);
...
app.post("/api/setup", async (context) => {
  ...
  if (current?.status === "ready")
    return context.json({ error: "Setup is closed." }, 409);
...
app.get("/api/attachments/:attachmentId", async (context) => {
  ...
  if (!object)
    return context.json({ error: "Attachment data is missing." }, 404);
...
app.onError((error, context) => {
  ...
  if (error instanceof z.ZodError)
    return context.json(
      { error: "The request data is invalid.", details: error.issues },
      400
    );
  ...
  async queue(batch: MessageBatch<unknown>, env: Env): Promise<void> {
    for (const item of batch.messages) {
      try {
        ...
        if (job.type === "parse-inbound")
          await processInbound(env, job.messageId);
...
async function requireUser(env: Env, request: Request): Promise<UserContext> {
  ...
  if (!session?.user)
    throw new ResponseError(401, "Authentication is required.");
  const member = await env.MAIL_DB.prepare(
    "SELECT role FROM mail_member WHERE user_id = ?"
  )
    .bind(session.user.id)
    .first<{ role: "owner" | "member" }>();
  if (!member)
    throw new ResponseError(403, "This account is not a mail member.");
  return {
    id: session.user.id,
...
async function requireOwner(env: Env, request: Request): Promise<UserContext> {
  ...
  if (user.role !== "owner")
    throw new ResponseError(403, "Owner access is required.");
  return user;
}
...
function requiredQuery(value: string | undefined, name: string): string {
  if (!value)
    throw new ResponseError(400, `Query parameter ${name} is required.`);
  return value;
}
