...
export const BindingsSchema = z.object({
  ...
  AI: z
    ...
    .optional(),
  FLARY_INTERNAL_TOKEN: z.string(),
  FLARY_SESSION_ARCHIVE_KEY: z.string(),
  FLARY_TOKEN_ENCRYPTION_KEY_B64: z.string(),
  FLARY_SETUP_TOKEN: z.string(),
  BETTER_AUTH_SECRET: z.string(),
  FLARY_DASHBOARD_DB: z.custom<D1Database>(),
  LOADER: z.unknown().optional(),
  FLARY_RUN_SERVICE: z.unknown().optional(),
  FLARY_THREAD_CONTROL: z.unknown().optional(),
});
...
export const app = flary({
  ...
  auth: async ({ request, bindings }) => {
    ...
    const session = await createAuth(bindings, new URL(request.url).origin).api.getSession({
