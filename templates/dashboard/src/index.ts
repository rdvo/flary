...
worker.on(["GET", "POST"], "/api/auth/*", (context) =>
...
worker.post("/api/connections/oauth/start", async (context) => {
  ...
  if (body.provider !== "openai-codex" && body.provider !== "anthropic")
...
worker.get("/api/connections/oauth/:id", async (context) => {
  const owner = await requireOwner(context.env, context.req.raw);
  return context.json(
    await providerOAuth(context.env).get(
      owner,
      context.req.param("id"),
      context.req.query("poll") === "1",
    ),
  );
});
worker.post("/api/connections/oauth/:id/complete", async (context) => {
  ...
  if (!body.authorizationResult)
    return context.json({ error: "Authorization result is required." }, 400);
  return context.json(
    await providerOAuth(context.env).complete(
      owner,
      context.req.param("id"),
...
worker.get("/api/connections/mcp/oauth/callback", async (context) => {
  ...
  if (!state)
    return context.html(
      mcpCallbackHtml(false, "The authorization response did not include state."),
      400,
    );
  try {
...
worker.post("/api/connections/mcp", async (context) => {
  ...
  try {
    return context.json(
      await mcpOAuth(context.env, context.req.raw).start(owner, {
        name: body.name,
...
worker.post("/api/setup", async (context) => {
  ...
  if (!body.token || !constantTimeEqual(body.token, context.env.FLARY_SETUP_TOKEN))
  ...
  if (current?.status === "initializing") {
    if (Date.now() - current.createdAt < 5 * 60_000)
      return context.json({ error: "Owner setup is already running." }, 409);
    const users = await context.env.FLARY_DASHBOARD_DB.prepare(
      "SELECT id, email FROM user ORDER BY created_at ASC LIMIT 2",
    ).all<{ id: string; email: string }>();
    if (users.results.length === 1 && users.results[0]?.email === body.email) {
      const owner = users.results[0]!;
      await context.env.FLARY_DASHBOARD_DB.prepare(
        "UPDATE flary_installation SET status = 'ready', owner_user_id = ?, initialized_at = unixepoch() * 1000 WHERE id = 'owner' AND status = 'initializing'",
      )
        .bind(owner.id)
        .run();
    ...
    if (users.results.length > 0)
      return context.json(
        { error: "Setup needs manual recovery because an unexpected user exists." },
        409,
      );
    await context.env.FLARY_DASHBOARD_DB.prepare(
      "DELETE FROM flary_installation WHERE id = 'owner' AND status = 'initializing'",
    ).run();
  }
  const lock = await context.env.FLARY_DASHBOARD_DB.prepare(
    "INSERT OR IGNORE INTO flary_installation (id, status, created_at) VALUES ('owner', 'initializing', unixepoch() * 1000)",
  ).run();
  ...
  try {
    const created = await createAuth(context.env, new URL(context.req.url).origin).api.signUpEmail({
      body: { name: body.name ?? "Owner", email: body.email ?? "", password: body.password ?? "" },
    });
    await context.env.FLARY_DASHBOARD_DB.prepare(
      "UPDATE flary_installation SET status = 'ready', owner_user_id = ?, initialized_at = unixepoch() * 1000 WHERE id = 'owner' AND status = 'initializing'",
    )
    ...
    const partialOwner =
      users.results.length === 1 && users.results[0]?.email === body.email
        ? users.results[0]
        : undefined;
    if (partialOwner) {
      await context.env.FLARY_DASHBOARD_DB.prepare(
    ...
    return context.json(
      { error: "Owner setup failed. Check the Worker logs, then try again." },
      400,
    );
  }
});
worker.get("/", async (context) => {
  ...
  const session = open
    ? undefined
    : await createAuth(context.env, new URL(context.req.url).origin).api.getSession({
        headers: context.req.raw.headers,
      });
  return context.html(dashboardHtml(open, Boolean(session?.user)));
});
worker.get("/setup", async (context) =>
  context.html(dashboardHtml((await installation(context.env))?.status !== "ready")),
);
...
async function installation(
  env: Bindings,
): Promise<{ status: string; createdAt: number } | undefined> {
  return (
    (await env.FLARY_DASHBOARD_DB.prepare(
      "SELECT status, created_at AS createdAt FROM flary_installation WHERE id = 'owner'",
    ).first<{ status: string; createdAt: number }>()) ?? undefined
  );
}
...
function providerOAuth(env: Bindings): CloudflareProviderOAuthPersistence {
  return new CloudflareProviderOAuthPersistence({
