...
export class DeterministicModelRouter {
  ...
  async decide(input: RoutingRequest): Promise<RouteDecision> {
    ...
    const candidates = input.candidates.map((candidate) => ProviderRouteCandidateSchema.parse(candidate));
    ...
    const scored = [
      ...allowed.map((selection, index) => scoreCandidate({
        selection,
    ...
    eligible.sort((left, right) => {
      ...
      return difference !== 0 ? difference : allowed.indexOf(left.selection) - allowed.indexOf(right.selection);
    });
    const selected = eligible[0]!;
    const decision = RouteDecisionSchema.parse({
      decisionId: `route_${hash(`${input.operationId}:${selected.selection.provider}/${selected.selection.model}`)}`,
   ...
   */
  async execute<T>(input: RoutingRequest & {
    ...
    try {
      ...
      const fallbackCandidates = uniqueModels(
        (input.policy.fallback ?? []).map((candidate) => normalizeModelInput(candidate as ModelInput)),
      );
      let fallbackIndex = 0;
...
function scoreCandidate(input: {
  ...
  if (candidate && policy.capabilities.some((capability) => !candidate.capabilities.includes(capability))) {
    eligible = false;
    reasons.push("missing_capability");
  }
  if (candidate?.costUsd !== undefined && policy.maxCostUsd !== undefined && candidate.costUsd > policy.maxCostUsd) {
    eligible = false;
    reasons.push("cost_limit");
  }
  if (candidate?.latencyMs !== undefined && policy.maxLatencyMs !== undefined && candidate.latencyMs > policy.maxLatencyMs) {
    eligible = false;
  ...
  return {
    selection,
    eligible,
    score: eligible && candidate ? score(policy, candidate, index, count) : -Number.MAX_SAFE_INTEGER,
    reason: reasons.length > 0 ? reasons.join(",") : "eligible",
  };
}

function uniqueModels(models: readonly ModelSelection[]): ModelSelection[] {
  return models.filter((candidate, index) =>
...
export function routeCandidatesFromDescriptors(
  ...
  return descriptors.map((descriptor) => ProviderRouteCandidateSchema.parse({
...
function score(
  ...
  if (strategy === "cost") return candidate.costUsd === undefined ? 0 - index : -candidate.costUsd * 1_000 - index;
  if (strategy === "latency") return candidate.latencyMs === undefined ? 0 - index : -candidate.latencyMs - index;
