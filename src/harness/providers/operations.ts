...
export interface ModelOperationHandlers<TBindings = unknown> {
  generateText?: (request: GenerateTextRequest, context: ModelOperationContext<TBindings>) => Promise<GenerateTextResult>;
  generateObject?: (request: GenerateObjectRequest, context: ModelOperationContext<TBindings>) => Promise<unknown>;
  embed?: (request: EmbedRequest, context: ModelOperationContext<TBindings>) => Promise<EmbedResult>;
  generateImage?: (request: GenerateImageRequest, context: ModelOperationContext<TBindings>) => Promise<ImageResult>;
  transcribe?: (request: TranscribeRequest, context: ModelOperationContext<TBindings>) => Promise<TranscriptionResult>;
  generateAudio?: (request: GenerateAudioRequest, context: ModelOperationContext<TBindings>) => Promise<AudioResult>;
  generateVideo?: (request: GenerateVideoRequest, context: ModelOperationContext<TBindings>) => Promise<VideoResult>;
  rerank?: (request: RerankRequest, context: ModelOperationContext<TBindings>) => Promise<RerankResult>;
  moderate?: (request: ModerateRequest, context: ModelOperationContext<TBindings>) => Promise<ModerationResult>;
}
...
export interface ModelOperations<TBindings = unknown> {
  generateText(request: GenerateTextRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<GenerateTextResult>;
  generateObject<TSchema extends ZodType>(request: GenerateObjectRequest<TSchema>, context?: Partial<ModelOperationContext<TBindings>>): Promise<z.output<TSchema>>;
  embed(request: EmbedRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<EmbedResult>;
  generateImage(request: GenerateImageRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<ImageResult>;
  transcribe(request: TranscribeRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<TranscriptionResult>;
  generateAudio(request: GenerateAudioRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<AudioResult>;
  generateVideo(request: GenerateVideoRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<VideoResult>;
  rerank(request: RerankRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<RerankResult>;
  moderate(request: ModerateRequest, context?: Partial<ModelOperationContext<TBindings>>): Promise<ModerationResult>;
}
...
export function createModelOperations<TBindings = unknown>(
  ...
  return {
    generateText: (request, context) =>
      operation("text", withDefault(request), GenerateTextRequestSchema, options.handlers?.generateText, context),
    generateObject: async (request, context) => {
      const parsed = GenerateObjectRequestSchema.parse(withDefault(request as Record<string, unknown>));
      const result = await operation("object", parsed, GenerateObjectRequestSchema, options.handlers?.generateObject, context);
      return parsed.schema.parse(result) as z.output<typeof request.schema>;
    },
    embed: (request, context) =>
      operation("embedding", withDefault(request), EmbedRequestSchema, options.handlers?.embed, context),
    generateImage: (request, context) =>
      operation("image", withDefault(request), GenerateImageRequestSchema, options.handlers?.generateImage, context),
    transcribe: (request, context) =>
      operation("transcription", withDefault(request), TranscribeRequestSchema, options.handlers?.transcribe, context),
    generateAudio: (request, context) =>
      operation("audio", withDefault(request), GenerateAudioRequestSchema, options.handlers?.generateAudio, context),
    generateVideo: (request, context) =>
      operation("video", withDefault(request), GenerateVideoRequestSchema, options.handlers?.generateVideo, context),
    rerank: (request, context) =>
      operation("rerank", withDefault(request), RerankRequestSchema, options.handlers?.rerank, context),
    moderate: (request, context) =>
      operation("moderation", withDefault(request), ModerateRequestSchema, options.handlers?.moderate, context),
  };
}
  ...
  const messages: ProviderMessage[] = request.prompt
    ...
    : request.messages!.map((message) => {
        if (!message || typeof message !== "object" || Array.isArray(message)) {
          throw new ModelOperationError("text", "message_invalid", "Every message must be an object");
        }
...
export function createAdapterOperationHandlers<TBindings = unknown>(options: {
  readonly resolveAdapter: (selection: ModelSelection, bindings: TBindings) => {
  ...
  return {
    ...
    generateObject: async (request, context) => {
      ...
      try {
        ...
        throw new ModelOperationError("object", "invalid_json", error instanceof Error ? error.message : "The provider returned invalid JSON");
      }
