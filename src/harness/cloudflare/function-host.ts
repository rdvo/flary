...
export function createFlaryDurableRunService(
  ...
  const stub = options.namespace.get(options.namespace.idFromName(options.name ?? "default"));
  ...
  return {
    create: (context, request) => call<RunHandle>("create", { context, request }),
    ...
    input: (context, runId, input) => call<RunResult>("input", { context, runId, input }),
    cancel: (context, runId, input) => call<RunResult>("cancel", { context, runId, input }),
  ...
  return {
      ...
      const response = await directFetch(
        ...
        )}/${encodeURIComponent(instanceId)}`,
        {
          ...
          body: JSON.stringify({
            ...
            ...(sendOptions.cacheRetention ? { cacheRetention: sendOptions.cacheRetention } : {}),
            ...(sendOptions.turnContext ? { turnContext: sendOptions.turnContext } : {}),
          }),
        },
      );
      const value = await response.json().catch(() => undefined);
      if (!response.ok) {
        const detail = isRecord(value) && isRecord(value.error) ? value.error.message : undefined;
        throw new Error(
    ...
    async delete(agentName, instanceId) {
      ...
      if (!response.ok) {
        ...
        const detail = isRecord(value) && isRecord(value.error) ? value.error.message : undefined;
        throw new Error(
          typeof detail === "string" ? detail : `Flue agent deletion failed (${response.status})`,
        );
      }
    },
 ...
 */
export function createFlaryCodemodeApprovalHooks<TEnv extends Record<string, unknown>>(
  env: TEnv,
  options: {
    readonly token?: string;
    readonly repository?: FlaryUserInputRepository;
  } = {},
): Pick<FlaryDurableRunObjectOptions<TEnv>, "createApprovalHooks">["createApprovalHooks"] {
  ...
  const request = async (
    ...
    const targetId = kind === "workflow" ? record.admission.submissionId : record.instanceId;
    const binding = bindingFor(env, kind, record.agentName);
    const path = `/${
      kind === "workflow" ? "workflows" : "agents"
    }/${encodeURIComponent(record.agentName)}/${encodeURIComponent(targetId)}?flary=${action}`;
  ...
  return (_env, repositoryInput) => ({
    async listApprovals(record) {
      ...
      const approvals = isRecord(value) && Array.isArray(value.approvals) ? value.approvals : [];
...
async function forwardToNamespace(
  ...
  return stub.fetch(
    new Request(
      `https://flue.internal${new URL(request.url).pathname}${new URL(request.url).search}`,
      request,
    ),
  );
}
...
function errorCode(cause: unknown): string {
  return isRecord(cause) && typeof cause.code === "string" ? cause.code : "flary_runtime_failed";
}

function errorStatus(cause: unknown): number {
  return isRecord(cause) && typeof cause.status === "number" ? cause.status : 500;
}
