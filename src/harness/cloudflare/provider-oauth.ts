...
export class CloudflareProviderOAuthPersistence {
  ...
  async start(
    ...
    await this.options.database
      ...
      .bind(
        ...
        flow.verificationUri ?? null,
        flow.userCode ?? null,
        flow.intervalSeconds ?? null,
        privateState,
        expiresAt,
        this.now().getTime(),
        this.now().getTime(),
  ...
  async get(
    ...
    if (poll && row.status === "pending" && row.provider === "openai-codex") {
      try {
        const state = await this.decrypt<ProviderOAuthPrivateState>(
          row.private_state,
          associatedData(scope, id, "flow"),
        );
        ...
        if (result.status === "ready") await this.finish(scope, row, result.credential);
          ...
          await this.options.database
            .prepare(
              "UPDATE flary_provider_oauth_session SET private_state = ?, interval_seconds = ?, updated_at = ? WHERE id = ? AND tenant_id = ? AND user_id = ? AND status = 'pending'",
            )
            .bind(
              encrypted,
  ...
  private async finish(
    ...
    const encrypted = await this.encrypt(
      credential,
      associatedData(scope, row.connection_id, "credential"),
    );
    ...
    await this.options.database
      ...
      .bind(subject ?? null, this.now().getTime(), row.id, scope.tenantId, scope.userId)
      .run();
  }
  ...
  private async setTerminal(
    ...
    await this.options.database
      .prepare(
        "UPDATE flary_provider_oauth_session SET status = ?, private_state = '', updated_at = ? WHERE id = ? AND status = 'pending'",
      )
      .bind(status, this.now().getTime(), row.id)
      .run();
  }
  ...
  private async load(scope: CloudflareProviderOAuthScope, id: string): Promise<OAuthRow> {
    const row = await this.options.database
      .prepare(
        "SELECT * FROM flary_provider_oauth_session WHERE id = ? AND tenant_id = ? AND user_id = ?",
      )
  ...
  private async loadConnection(
    ...
    const row = await this.options.database
      ...
      .bind(connectionId, scope.tenantId, scope.userId)
      .first<ConnectionRow>();
  ...
  private async acquireRefreshLock(
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      const result = await this.options.database
        ...
        .bind(connectionId, lockId, now + 30_000)
        .run();
...
function safeOAuthError(error: unknown): Error {
  return new Error(error instanceof Error ? error.message : "The provider login failed");
}
