...
function assertOpenApiDocument(value: unknown): Record<string, unknown> {
  ...
  if (!isRecord(value.info) || typeof value.info.title !== "string" ||
...
function validateRequest(
  ...
  const result = new Validator(schema, spec.openapi?.toString().startsWith("3.1") ? "2020-12" : "7", false).validate(value);
  if (!result.valid) {
    throw new FlaryOpenApiValidationError(
      `OpenAPI ${operation.method.toUpperCase()} ${operation.path} request failed schema validation`,
    );
  }
}
...
function validateResponse(
  ...
  const schema = isRecord(jsonContent) && (isRecord(jsonContent.schema) || typeof jsonContent.schema === "boolean")
  ...
  if (!result.valid) {
    throw new FlaryOpenApiValidationError(
      `OpenAPI ${operation.method.toUpperCase()} ${operation.path} response failed schema validation`,
    );
  }
}
...
function requestSchema(
  ...
  const pathItem = isRecord(spec.paths)
    ...
    : undefined;
  const pathParameters = isRecord(pathItem) && Array.isArray(pathItem.parameters)
    ? pathItem.parameters
    : [];
  const parameters = [
    ...pathParameters,
  ...
  for (const parameter of parameters) {
    ...
    const parameterSchema = isRecord(resolvedParameter.schema) ||
...
function requestBodySchema(
  ...
  if (!isRecord(json) ||
      (!isRecord(json.schema) && typeof json.schema !== "boolean")) return undefined;
...
function pathParameterNames(
  ...
  return new Set(values.filter(isRecord).map((value) =>
    resolveDocumentObject(spec, value),
...
function resolveSchema(
    ...
    return Object.fromEntries(Object.entries(schema).map(([key, value]) => {
      if (key === "properties" && isRecord(value)) {
        return [key, Object.fromEntries(Object.entries(value).map(([name, child]) =>
          [name, isRecord(child) || typeof child === "boolean"
  ...
  const target = ref.slice(2).split("/").reduce<unknown>((current, part) =>
  ...
  return isRecord(resolved)
    ? { ...resolved, ...Object.fromEntries(Object.entries(schema).filter(([key]) => key !== "$ref")) }
    : resolved;
}
