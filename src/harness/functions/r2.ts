...
import type { FlaryR2Source, FlaryStepContext, FlaryToolConnection } from "./types.js";
import {
...
class R2FileConnection implements FlaryToolConnection {
  ...
  async call(name: string, input: unknown): Promise<unknown> {
    switch (name) {
      ...
      case "batchEdit":
        return this.batchEdit(input);
      case "move":
        return this.move(input);
      case "delete":
        return this.remove(input);
      default:
        throw new Error(`R2 file tool '${name}' is not available`);
    }
  }
  ...
  private path(key: string): string | undefined {
    if (this.basePrefix) {
      ...
      return relative && ProjectFilePathSchema.safeParse(relative).success ? relative : undefined;
    }
  ...
  private async entry(
    ...
    const sha256 =
      metadataHash && /^[0-9a-f]{64}$/.test(metadataHash)
        ? metadataHash
    ...
    return ProjectFileEntrySchema.parse({
      ...
      sha256,
      mediaType:
        object.httpMetadata?.contentType ?? object.customMetadata?.mediaType ?? mediaTypeFor(path),
      storage: "r2",
      createdAt: uploaded,
      updatedAt: uploaded,
    });
  ...
  private async list(input: unknown = {}) {
    ...
    const objects = await this.listObjects(
      this.directoryKey(request.prefix),
      request.limit,
      !request.recursive,
    );
  ...
  private async grep(input: unknown) {
    ...
    const pattern = request.regex
      ? new RegExp(request.query, request.caseSensitive ? "g" : "gi")
      : undefined;
    for (const file of files.files) {
      ...
      for (let index = 0; index < lines.length && matches.length < request.maxMatches; index += 1) {
        ...
        const found = pattern
          ...
          : request.wholeWord
            ? new RegExp(
                `\\b${escapeRegExp(request.query)}\\b`,
                request.caseSensitive ? "" : "i",
              ).test(lineText)
            : request.caseSensitive
              ? lineText.includes(request.query)
              : lineText.toLowerCase().includes(request.query.toLowerCase());
        if (!found) continue;
        const column = pattern
          ? Math.max(0, lineText.search(pattern))
          : Math.max(
              0,
              (request.caseSensitive ? lineText : lineText.toLowerCase()).indexOf(
                request.caseSensitive ? request.query : request.query.toLowerCase(),
              ),
            );
        matches.push({
          line: index + 1,
          ...
          ...(request.contextBefore > 0
            ? { beforeLines: lines.slice(Math.max(0, index - request.contextBefore), index) }
            : {}),
          ...(request.contextAfter > 0
            ? { afterLines: lines.slice(index + 1, index + 1 + request.contextAfter) }
            : {}),
        });
      }
  ...
  private async diff(input: unknown) {
    ...
    const target =
      request.newContent ??
      (await this.read({ path: request.compareToPath!, encoding: "utf8" })).content;
  ...
  private async edit(input: unknown) {
    ...
    for (const edit of request.edits) {
      ...
      if (!edit.replaceAll && matches > 1)
        throw new Error(`Edit text is ambiguous in ${request.path}`);
  ...
  private async remove(input: unknown) {
    ...
    const keys = request.recursive
      ? (await this.listObjects(prefix, MAX_LIST_FILES, false))
