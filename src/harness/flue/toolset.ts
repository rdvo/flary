...
import { CodeExecutionRouter, type CodeExecutionAdapter } from "../execution/adapters.js";
...
import { registerWorkspaceTools, type WorkspaceToolTarget } from "../tools/workspace.js";
...
export type FlaryToolCapability = (typeof FLARY_TOOL_CAPABILITIES)[number];
...
export interface FlaryResolvedMcpConnection {
  ...
  readonly endpoint: Omit<
    ...
    "connectionId" | "organizationId" | "appId" | "credentialVersion"
  >;
...
export type FlaryResolvedConnection = FlaryResolvedMcpConnection | FlaryResolvedApiConnection;
...
export interface CreateFlaryToolsetOptions {
  ...
  readonly workspace?: FlaryWorkspaceBindingSource | FlaryWorkspaceToolsetOptions;
...
const toolApprovalContinuations = new WeakMap<ToolDefinition[], ApprovalContinuation>();
...
export async function createFlaryToolset(
  ...
  if (
    ...
    const target = await resolveWorkspaceTarget(workspace.target, scope, options.blobs);
    registerWorkspaceTools(catalog, target, {
      requireApprovalForWrites: workspace.requireApprovalForWrites ?? true,
    });
  }
  ...
  if (options.approvals?.continuation) {
    toolApprovalContinuations.set(tools, options.approvals.continuation);
  }
  return {
    tools,
    catalog,
...
async function resolveCodeMode(
  ...
  if (!options?.enabled || options.adapter || options.execute) {
    return options;
  }
  try {
    ...
    const { createCloudflareCodeMode } = await import("../cloudflare/dynamic-worker.js");
    return createCloudflareCodeMode({
      loader: loader as never,
      timeoutMs: options.timeoutMs,
    });
  } catch {
    throw new Error("Code Mode needs a Dynamic Worker adapter or a Cloudflare LOADER binding");
  }
}

function normalizeWorkspaceOptions(
  value: FlaryWorkspaceBindingSource | FlaryWorkspaceToolsetOptions,
...
async function resolveWorkspaceTarget(
  ...
  const { createCloudflareWorkspaceTarget } = await import("../cloudflare/workspace.js");
...
function capabilityMode(capabilities: ReadonlySet<FlaryToolCapability>): AgentMode {
  return {
    id: "flary-toolset",
    name: "Flary toolset",
...
async function registerConnections(
  ...
  for (const id of ids) {
    ...
    if (connection.kind === "mcp") {
      if (!capabilities.has("mcp.call") && !capabilities.has("mcp.search")) {
        continue;
      }
      const endpoint: ScopedMcpEndpointInput = {
        ...
        appId: scope.appId,
        ...(connection.revision ? { credentialVersion: connection.revision } : {}),
      };
      const toolset = await createMcpToolset({
        ...
        permissions: connection.permissions ?? defaultMcpPermissions(capabilities),
...
function requiredMcpCredentials(
  ...
  return async (request) => {
    const value =
      typeof resolver === "function" ? await resolver(request) : await resolver.get(request);
    if (!value) {
      throw new McpSecurityError("The authorized MCP credential is not available");
    }
    return value;
  };
}
...
function defaultMcpPermissions(
  ...
  return ({ tool }): McpToolGrantInput | false => {
    ...
    return {
      ...
      requiresApproval: operation === "write" || Boolean(tool.annotations?.destructiveHint),
    };
  };
}
...
async function registerApiConnection(
  ...
  for (const descriptor of descriptors) {
    ...
    const inputValidator = new Validator(descriptor.inputSchema as JsonSchema);
    ...
    catalog.register({
      definition: {
        ...
        description: descriptor.description ?? "Call an authorized API operation.",
        kind: "http",
        ...
        requiresApproval: descriptor.requiresApproval ?? operation === "write",
...
function createCodeModeTool(input: {
  ...
  return defineTool({
    ...
    async run({ input: request }) {
      ...
      const executionKey = await codeExecutionKey(input.scope, parsed.code);
      const tools = runtimeBridges(input.runtime, executionKey);
      if (input.options.execute) {
        return toJson(
          await input.options.execute({
            code: parsed.code,
            scope: input.scope,
            tools,
          }),
      ...
      const result = await router.execute(
        {
          ...
          runId:
            input.scope.runId ?? input.scope.threadId ?? `workspace_${input.scope.workspaceId}`,
          ...
          toolNamespaces: [
            {
              ...
              tools: {
                ...
                call: (id, value) => tools.call(String(id), JsonObjectSchema.parse(value ?? {})),
                batch: (calls) =>
                  tools.batch(
                    z
                      .array(
...
function runtimeBridges(runtime: LazyToolRuntime, executionKey: string) {
  let ordinal = 0;
  const nextCallId = () => `${executionKey}_${++ordinal}`;
  return {
    search: async (query: string) => toJson(await runtime.search({ query, limit: 10 })),
    describe: async (id: string) => toJson(await runtime.describe(id)),
...
async function codeExecutionKey(
  ...
  const digest = await crypto.subtle.digest(
    ...
    new TextEncoder().encode(
      [scope.runId ?? scope.threadId ?? scope.workspaceId, code].join("\u0000"),
    ),
  );
...
async function defaultJournal(): Promise<ToolExecutionJournal> {
  try {
    ...
    if (storage.sql) {
      const { SqliteToolExecutionJournal } = await import("../cloudflare/tool-journal.js");
