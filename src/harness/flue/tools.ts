...
import { waitForUserInput } from "../tools/user-input.js";
import {
...
export interface FlueUserInputToolOptions {
  ...
  waitForResponse?: (request: UserInputRequest, signal?: AbortSignal) => Promise<UserInputResponse>;
}

export interface FlueSecretRequestToolOptions {
  createRequest(input: CollectApiKeyRequest): Promise<UserInputRequest> | UserInputRequest;
  waitForResponse(request: UserInputRequest, signal?: AbortSignal): Promise<SecretRequestResult>;
}
 ...
 */
export function createFlueLazyTools(runtime: LazyToolRuntime): ToolDefinition[] {
  ...
  const describe = defineTool({
    name: "tool_describe",
    description: "Load the input schema and policy for one tool returned by tool_search.",
 ...
 */
export function createFlueRequestUserInputTool(options: FlueUserInputToolOptions): ToolDefinition {
  return defineTool({
    ...
    async run({ input, signal }) {
      const questions = UserInputQuestionSchema.array()
        .min(1)
        .max(3)
        .parse(
      ...
      const request = UserInputRequestSchema.parse(await options.createRequest({ questions }));
 ...
 */
export function createFlueRequestSecretTool(options: FlueSecretRequestToolOptions): ToolDefinition {
  return defineTool({
    name: "request_secret",
    description:
    ...
    input: v.object({
      ...
      scope: v.optional(
        v.picklist(["run", "agent", "flow", "workspace", "project", "organization"]),
      ),
    }),
    async run({ input, signal }) {
