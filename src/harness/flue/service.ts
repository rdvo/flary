...
export interface FlueAgentGateway {
  ...
  invokeWorkflow?(workflowName: string, input: unknown): Promise<FlueAdmission>;
...
export function createFlueRunService(
  ...
  const refreshWaiting = async (
    ...
    await options.repository.appendEvent(
      record.runId,
      `waiting:${approval ? "approval" : "input"}:${
          ...
          cost:
            subscription && chunk.usage.cost.total === 0
              ...
              : {
                  ...
                  microUnits: Math.max(0, Math.round(costUsd * 1_000_000)),
                  unit: "USD" as const,
                },
          ...(subscription && chunk.usage.cost.total === 0 ? {} : { costUsd }),
        };
      }
      if (!mapped) return;
      await options.repository.appendEvent(
    ...
    case "tool-output-error": {
      ...
      return eventDraft(record, "tool.result", {
        result: {
          ...
          toolId,
          status: chunk.type === "tool-output" ? "succeeded" : "failed",
        ...
        ...(chunk.usage
          ? {
              usage: {
                ...
                cost:
                  billingMode === "subscription" && chunk.usage.cost.total === 0
                    ? {
                        state: "unknown" as const,
                        reason: "Subscription provider did not report a cost",
                      }
...
function sameScope(left: TrustedRunContext, right: TrustedRunContext): boolean {
  return (
