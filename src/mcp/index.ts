  ...
  constructor(config: MCPConfig) {
    ...
    this.#app.use("*", async (c, next) => {
      if (this.#config.auth?.type === "bearer") {
        ...
        if (!token) {
          return new Response(
            "Unauthorized - Bearer token required in header or key parameter",
            { status: 401 }
          );
        }
  ...
  tool<T extends z.ZodObject<any>>(
    ...
    if (typeof schemaOrHandler === "function" && !handler) {
      ...
      if (!fn.schema || typeof fn.schema !== "object") {
        throw new Error(
          `Function ${name} must have a schema property when used directly`
        );
      }
      schema = fn.schema;
      finalHandler = fn;
    ...
    this.#server.tool(name, schema.shape as any, (args: any, extra: any) =>
      wrappedHandler(args, extra)
    );
    return this;
  }
...
class MCPDurableObject {
  ...
  private async setupSSETransport(url: URL): Promise<EdgeSSETransport> {
    ...
    console.log(
      `[MCP] Current sseTransport:`,
      this.sseTransport ? "exists" : "null"
    );
    ...
    if (!this.sseTransport) {
      ...
      console.log(
        `[MCP] Creating new SSE transport with messageUrl: ${messageUrl.toString()}`
      );
      this.sseTransport = new EdgeSSETransport(
        messageUrl.toString(),
      ...
      if (!this.sseTransport.onmessage) {
        console.log(
          `[MCP] Re-setting onmessage handler for existing SSE transport`
        );
        this.sseTransport.onmessage = async (message) => {
          console.log(`[MCP] Received message from client:`, message);
          try {
  ...
  private async setupWSTransport(
    url: URL,
    connectionId: string
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (isWebSocketRequest && (isSpecificWsEndpoint || pathname === "/")) {
        ...
        const connectionId =
          this.state.id.toString() + "-" + Date.now().toString();
      ...
      if (
        ...
        if (!urlSessionId) {
          console.log(
            `[MCP] No sessionId provided, redirecting to URL with sessionId: ${sessionId}`
          );
      ...
      if (
        request.method === "POST" &&
        (pathname.endsWith("/message") || pathname === "/")
      ) {
        ...
        try {
          ...
          const error = err as Error;
          console.error(
            `[MCP] Error connecting SSE transport for POST:`,
            error
          );
          return new Response(`SSE connection error: ${error.message}`, {
            status: 500,
        ...
        return new Response(
          JSON.stringify({
            ...
            backward_compatibility: {
              sse: this.createCompatibilityUrl(
                url,
                "/sse",
                this.state.id.toString()
              ),
              websocket: this.createCompatibilityUrl(
                url,
                "/ws",
                this.state.id.toString()
              ),
              message: this.createCompatibilityUrl(
                url,
                "/message",
                this.state.id.toString()
              ),
  ...
  private createCompatibilityUrl(
    url: URL,
    endpoint: string,
    sessionId: string
  ): string {
